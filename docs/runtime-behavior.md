...
按鍵綁定區的熱鍵只負責觸發工具功能；`GameKeys` 區段則決定 A／S／D 連點與抖槍術實際送入遊戲的鍵盤按鍵。沒有 `GameKeys` 區段的舊設定檔必須維持原始預設：A／S／D 連點輸出 `a`／`s`／`d`，抖槍術輸出 `z`／`x`／`c`／`v`／`b`。

遊戲鍵位設定只捕捉鍵盤，不捕捉滑鼠。八個輸出鍵不可共用，變更時先保留在對話框草稿，只有按下「儲存」且所有鍵位皆可輸出、互不重複時才寫入設定並立即生效；「取消」不得修改正式設定。
